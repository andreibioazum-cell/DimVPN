...
function initPeer() {
    ...
    peer.on('error', (err) => {
        ...
        if (err.type === 'peer-unavailable') {
            alert('Узел не найден. Проверьте правильность ID.');
        } else if (err.type === 'unavailable-id') {
            alert('ID занят или ошибка сети. Перезагрузите страницу.');
        } else {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('connect-btn').onclick = () => {
        ...
        // Кнопка разблокируется через системный peer.on('error') или при открытии
        conn.on('open', () => {
            connectBtn.disabled = false;
            connectBtn.innerText = 'УСТАНОВИТЬ СВЯЗЬ';
            input.value = '';
